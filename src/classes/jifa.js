export default function asd(AnimeName, elementContainer, videoURL) {
    ...
    function asdf(AnimeName) {
        var video = container.querySelector("#guydhtVideoMyVideo"),
            ...
            quality = container.querySelector("#guydhtVideoQuality"),
            ...
            subText = container.querySelector("#guydhtVideoSubsText");
        if (upperName.children[0].value.includes("http"))
            upperName.children[0].value = "Name Unknown";
        if (AnimeName != null && !AnimeName.includes("http")) {
            var searchName = AnimeName + " Progress";
            video.addEventListener("timeupdate", function() {
                var obj = {},
                    searchName = AnimeName + " Progress";
                obj[searchName] = Math.round(this.currentTime / this.duration * 1000) / 1000;
                if (video.subs != null) {
                    var showSubsFlag = false,
                        timeInMili = video.currentTime * 1000;
                    for (var obj of video.subs) {
                        if (obj.end >= timeInMili && obj.start <= timeInMili) {
                            subText.innerHTML = obj.text;
                            showSubsFlag = true;
                            break;
                        }
                    }
        ...
        video.addEventListener("loadedmetadata", function() {
            ...
            if (video.videoHeight > video.videoWidth) {
                video.style = `transform: rotate(-90deg) scale(${ratio});`
                ...
                ["Search Online", function() {
                    ...
                    then(async response => {
                        ...
                        if (response.responseURL.includes("captcha"))
                            ...
                            var html = document.createElement("html");
                            html.innerHTML = response.responseText;
                            console.log(response.responseURL, html.querySelectorAll("a.bnone"), encodeURI("https://www.opensubtitles.org/en/search2?MovieName=" + searchName.replace(/\s/g, "+") + " &id=8&action=search&SubLanguageID=eng&SubLanguageID=eng&Season=&Episode=&SubSumCD=&Genre=&MovieByteSize=&MovieLanguage=&MovieImdbRatingSign=1&MovieImdbRating=&MovieCountry=&MovieYearSign=1&MovieYear=&MovieFPS=&SubFormat=&SubAddDate=&Uploader=&IDUser=&Translator=&IMDBID=&MovieHash=&IDMovie="));
                            fetch(html.querySelector("a.bnone").href.replace(window.location.origin, "https://opensubtitles.org/")).
                            then(async response => {
                                response.responseURL = response.url;
                                response.responseText = await response.text();
                                html.innerHTML = response.responseText;
        ...
        function setSecondScreen(arr) { //[["text", onclick]]
            secondSettingsList.children.slice(1).remove();
            for (var i of arr) {
        ...
        function loadQualityWhenReady() {
            video.addEventListener("loadedmetadata", function() {
                quality.dataset.text = Math.floor(video.videoWidth * 9 / 16) + "p";
            }, {
                once: true
            });
        }
        ...
        document.onwebkitfullscreenchange = function() {
            if (document.webkitFullscreenElement == null && wrapper.style.position == "fixed") exitFullscreenMode();
        };
        ...
        function enterFullscreenMode() {
            ...
            previousPosition = wrapper.getBoundingClientRect().toObj();
        ...
        function onkeydown(e) {
            if (e.target.tagName.toLowerCase() != "input" && e.code == "Backquote" && document.webkitFullscreenElement == null && document.pointerLockElement == null) {
        ...
        wrapper.onkeydown = function(e) {
            ...
            if (e.code == "KeyF" && ctrlShiftAlt && e.guydhtSentThis == null) {
        ...
        intervalArray.push(setInterval(setTimer, 250));
        ...
        function setTimer() {
            ...
            currentParts = progressContainer.querySelectorAll(".guydhtVideoBuffered");
            for (var i = 0; i < buffered.length; i++) {
                var start = buffered.start(i) * 100 / totalTime,
                    size = (buffered.end(i) - buffered.start(i)) * 100 / totalTime;
        ...
        progressContainer.onmousemove = function(e) {
            ...
            var style = progressContainer.getBoundingClientRect(),
                position = e.clientX - parseInt(style.left),
            ...
            if (thumbnailSec != previousThumbSec) {
                ...
                if (thumbnailsObj[thumbnailSec] == null || thumbnailsObj[thumbnailSec] == "waiting") {
                ...
                previousThumbSec = thumbnailSec;
                if (thumbnailsObj[thumbnailSec] == null) {
                    thumbnailsObj[thumbnailSec] = "waiting";
        ...
        var clonedVideo = video != null ? video.cloneNode(true) : null,
            queue = [],
            cheapFactor = true;
        clonedVideo.pause();
        ...
        function brightnessOfCanvas(canvas) {
            try {
                var sum = 0,
                    ctx = canvas.getContext("2d").getImageData(0, 0, canvas.width, canvas.height).data;
            } catch (e) {
                return null;
            }
            for (var i = 0, len = ctx.length; i < len; i += 4)
                sum += (ctx[i] + ctx[i + 1] + ctx[i + 2]) / 3;
        ...
        wrapper.onmouseup = function(e) {
            if (e.button != 0 || document.pointerLockElement != null) return false;
            longPauseTimeout = setTimeout(function() {
                longPauseFlag = false;
            }, 400);
        ...
        video.addEventListener("guydhtVideoMuteChange", function(e) {
            if (e.detail != null && e.detail.state == true) {
        ...
        var checkForBuffer, seekTimeout;
        ...
        var middleTooltipTimeout, loadingTimeout;

        function displayMiddleTooltip(text) {
            if (mouseIsDown || text === "" || text == null) {
                setTimeout(function() {
                    try {
...
function absPosition(element) {
    var bound = element.getBoundingClientRect().toObj();
    ...
    var windowTop = Number(window.screenY),
        ...
        windowRight = Number(screen.width - (windowLeft + windowWidth)),
        windowBottom = Number(screen.height - (windowTop + windowHeight));
    if (window != top && window.guydhtPosition != null) {
        windowHeight = window.guydhtPosition.height;
        windowWidth = window.guydhtPosition.width;
        windowTop += window.guydhtPosition.top;
        windowLeft += window.guydhtPosition.left;
        windowRight = Number(screen.width - (windowLeft + windowWidth));
        windowBottom = Number(screen.height - (windowTop + windowHeight))
    }
    return {
...
function waitFor(condition, myFunction, amount, ...args) {
    ...
    else interval = setInterval(check, amount != null ? amount : 500, condition, myFunction, ...args);
